...
export default function SevenZipExtractorPage() {
  const [sevenZipFile, setSevenZipFile] = useState(null);
  ...
  const handleFileUploaded = async (file) => {
    setProcessing(true);
    setStatusText("Loading 7-Zip WebAssembly engine...");
    setSevenZipFile(file);
    ...
    try {
      ...
      setStatusText("Reading 7z archive...");
      const arrayBuffer = await file.arrayBuffer();
      
      // Write the file to Emscripten memory file system
      sevenZip.FS.writeFile('/archive.7z', new Uint8Array(arrayBuffer));
      ...
      sevenZip.callMain(['x', '/archive.7z', '-o/output', '-y']);
      ...
      if (files.length === 0) {
        throw new Error("No files extracted. Verify if the file is a valid 7Z archive.");
      }
      ...
      alert("Failed to extract 7Z archive: " + error.message);
      handleReset();
    } finally {
      setProcessing(false);
      setStatusText("");
  ...
  return (
    <FileToolLayout
      title="7Z Extractor"
      description="Inspect and extract files from 7-Zip archives locally inside the browser using WebAssembly."
      sidebarControls={sidebarControls}
      onFileUploaded={handleFileUploaded}
      onReset={handleReset}
      originalFile={sevenZipFile}
        ...
        <div className="p-4 flex-grow overflow-y-auto max-h-[360px] space-y-2">
          {processing && statusText !== "" ? (
            ...
            <div className="h-full min-h-[260px] flex flex-col items-center justify-center text-center">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-xs font-semibold">Drop or select a 7Z archive to unpack.</p>
