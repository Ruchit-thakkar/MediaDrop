...
export default function FileToolLayout({
  title,
  description,
  sidebarControls,
  onFileUploaded, // for single file tools
  onFilesUploaded, // for multi-file tools
  onReset,
  onDownload,
  ...
  // Helper to extract files recursively from directory entries
  const traverseDirectory = async (entry) => {
    const files = [];
    if (entry.isFile) {
      const file = await new Promise((resolve) => entry.file(resolve));
      // Store relative path for directory retention
    ...
    if (e.dataTransfer.items) {
      ...
      if (entryPromises.length > 0) {
        ...
        if (allFiles.length > 0) {
          if (allowMultiple) {
            if (onFilesUploaded) onFilesUploaded(allFiles);
          } else {
            if (onFileUploaded) onFileUploaded(allFiles[0]);
          }
        }
      }
      ...
      if (allowMultiple) {
        ...
        if (onFileUploaded) onFileUploaded(files[0]);
      }
    }
  };
  ...
  const totalUploadedSize = allowMultiple
    ? uploadedFiles.reduce((acc, curr) => acc + (curr.size || 0), 0)
    : (originalFile?.size || 0);
      ...
      <main className="pt-28 pb-20 flex-grow max-w-6xl w-full mx-auto px-6">
        ...
        <div className="mb-6 animate-fade-in">
          <nav className="flex flex-wrap items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
            ...
            <span className="material-symbols-outlined text-[12px] select-none">chevron_right</span>
            <Link href="/tools/file-tools" className="hover:text-purple-500 transition-colors text-zinc-500 dark:text-zinc-400">File Tools</Link>
            <span className="material-symbols-outlined text-[12px] select-none">chevron_right</span>
              ...
              >
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="upload-title">
                    {directoryMode 
                      ? "Click to select a folder or drag it here" 
                      : `Click to upload or drag ${allowMultiple ? "files" : "a file"}`}
                  </p>
                  <p className="upload-hint">
                    {directoryMode 
                      ? "Traverses subdirectories · processed 100% locally"
                      : "Privacy first · all files processed completely in-browser"}
                  </p>
                </div>
              </div>
            ) : (
                ...
                <div className="actions-sidebar">
                  <div className="actions-card text-center lg:text-left flex flex-col h-full justify-between">
                      ...
                      {hasFiles && (
                        <div className="mt-6 pt-4 border-t border-border-subtle text-[11px] font-bold text-zinc-500 dark:text-zinc-400 space-y-2 select-none">
                          <div className="flex justify-between">
                            <span>{allowMultiple ? "Total Files:" : "Filename:"}</span>
                            <span className="font-mono text-zinc-800 dark:text-zinc-200 truncate max-w-[120px]">
                              {allowMultiple ? uploadedFiles.length : (originalFile?.name || "File")}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>Input Size:</span>
                            <span className="font-mono text-zinc-800 dark:text-zinc-200">{formatBytes(totalUploadedSize)}</span>
                          </div>
                          {processedSize !== null && processedSize !== undefined && (
                            <div className="flex justify-between">
                              <span>Output Size:</span>
                              <span className="font-mono text-zinc-800 dark:text-zinc-200">{formatBytes(processedSize)}</span>
                            </div>
                          )}
                          {totalUploadedSize > 0 && processedSize !== null && processedSize !== undefined && (
                            <div className="flex justify-between text-purple-500">
                              <span>Ratio / Savings:</span>
                              <span>
                                {processedSize > totalUploadedSize 
                                  ? `${Math.round((processedSize / totalUploadedSize) * 100)}%`
                                  : `${Math.max(0, Math.round(((totalUploadedSize - processedSize) / totalUploadedSize) * 100))}% savings`}
                              </span>
                            </div>
                          )}
                        </div>
                    ...
                    <div className="mt-8 pt-4 border-t border-border-subtle space-y-3">
                      {/* Download/Action Button (Only rendered if onDownload is provided) */}
                      {onDownload && (
                        <button
                          onClick={onDownload}
