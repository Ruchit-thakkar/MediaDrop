...
export default function VideoTrimmerPage() {
  ...
  const startTrimming = async () => {
    ...
    try {
      ...
      console.warn("Could not capture audio for trimmer:", e);
    }
    ...
    const drawFrame = () => {
      ...
      const currentProgress = Math.min(99, Math.round((elapsed / trimDuration) * 100));
  ...
  const handleDownload = () => {
    ...
    a.download = `${baseName}_trimmed.webm`;
  ...
  return (
    <VideoToolLayout
      title="Video Trimmer"
      description="Trim or cut a section of your video file locally with precise frame timeline ranges."
          ...
          <div className="p-4 flex items-center justify-center flex-grow bg-zinc-950/40 overflow-hidden relative">
            {processing ? (
              <div className="flex flex-col items-center justify-center w-full h-full bg-zinc-950/60 absolute inset-0 z-10">
                ...
                <p className="text-xs font-semibold text-purple-400">Trimming video... {progress}%</p>
                <button
            ...
            {trimmedUrl ? (
              ...
              <div className="text-center">
                <p className="text-xs font-semibold text-zinc-400 font-semibold leading-relaxed">
                  Select start and end points in the sidebar, then click 'Trim Video'
                </p>
              </div>
            )}
          </div>
