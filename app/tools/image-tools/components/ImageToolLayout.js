  ...
  const handleDrop = (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      if (allowMultiple) {
        ...
        if (onFileUploaded && e.dataTransfer.files[0].type.startsWith("image/")) {
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      if (allowMultiple) {
        ...
        if (onFileUploaded && e.target.files[0].type.startsWith("image/")) {
      ...
      <main className="pt-28 pb-20 flex-grow max-w-6xl w-full mx-auto px-6">
        ...
        <div className="mb-6 animate-fade-in">
          <nav className="flex flex-wrap items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
            ...
            <span className="material-symbols-outlined text-[12px] select-none">chevron_right</span>
            <Link href="/tools/image-tools" className="hover:text-purple-500 transition-colors">Image Tools</Link>
            <span className="material-symbols-outlined text-[12px] select-none">chevron_right</span>
        ...
        <section className="max-w-5xl w-full mx-auto relative z-10 animate-fade-in">
          <input
            ...
            accept={title === "HEIC to JPG Converter" ? ".heic" : "image/*"}
              ...
              >
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="upload-hint">
                    {title === "HEIC to JPG Converter" ? "HEIC only" : "PNG, JPG, WebP, SVG, GIF"} · processed 100% locally
                  </p>
                </div>
              </div>
            ) : (
